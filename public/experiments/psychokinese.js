...
/* EDIT: CONFIG (Farben, Look & Größen) */
  ...
  .chart-legend{display:flex;flex-wrap:wrap;gap:12px;margin:10px 0;color:var(--muted);font-size:13px;align-items:center}
  .legend-item{display:flex;align-items:center;gap:6px;white-space:nowrap}
  .legend-swatch{width:28px;display:inline-block;flex-shrink:0;border-radius:999px;border:1px solid transparent}
  .legend-swatch.line{height:4px;background:currentColor;box-shadow:0 0 0 1px rgba(12,18,22,0.4)}
  .legend-swatch.fill{height:12px}
  ...
  .result-target{display:flex;flex-direction:column;align-items:center;gap:10px;margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.08);width:100%}
  .result-target-info{margin:0;color:var(--muted);font-size:13px;text-align:center;max-width:560px}
  @media(min-width:768px){
    .result-key{flex-direction:row;gap:18px}
    .result-accuracy{font-size:64px}
    .result-target{flex-direction:row;justify-content:center;text-align:left}
    .result-target-info{text-align:left}
  }
...
function setupExperiment(context) {
  const services = context?.services || {};
  const submit = typeof services.submit === 'function'
    ? services.submit
    : payload => {
        if (typeof window.submitExperimentData === 'function') {
          window.submitExperimentData(payload);
        }
      };
  const storageKey = context?.storageKey || 'psi::psychokinese::sessions';
...
function stopExp(autoTriggered=false){
  ...
  if(uploadPayload) submit(uploadPayload);
}
function resetExp(){
...
return () => {
  try { cancelAnimationFrame(timer); } catch (error) {}
...
export function mount({ container, context = {} }) {
  ...
  const cleanup = setupExperiment(context) || (() => {});
  return () => {
